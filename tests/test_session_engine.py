...
class TestGroupAccess:
    def test_dynamic_role_grants_group_access(self, app_config):
        manifest = AppManifest(
            name="test",
            access=AccessPolicy(commands={}, dm=["hosts"], group=["session_participants"]),
        )
        ctrl = AccessControl(manifest, app_config)
        player_jid = "player@s.whatsapp.net"
        ctrl.grant("session_participants", [{"jid": player_jid, "token": "Player"}])
